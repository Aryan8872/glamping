    ...
    <section id="tours" className="relative min-h-[60vh] py-20 bg-white" style={{ contentVisibility: 'auto', containIntrinsicSize: '800px' }}>
      <div className="mx-auto w-[92%] max-w-[1200px]">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} className="mb-8 text-center" variants={stagger(0.1)}>
          <motion.div variants={fadeInUp} className=" text-3xl font-bold uppercase tracking-[.3em] text-black">Popular Adventures</motion.div>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-100px' }} variants={stagger(0.15)} className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {tours.map((src, i) => (
            <motion.article
              key={i}
              variants={fadeInUp}
              whileHover={{ y: -10, scale: 1.05, boxShadow: '0 20px 40px rgba(0,0,0,0.4)', transition: { duration: 0.3, ease: [0.22, 1, 0.36, 1] } }}
            ...
            >
              <Link href={`/adventures/adventure${i + 1}`}>
                <img className="h-full w-full object-cover brightness-90" src={src} alt={`Tour ${i + 1}`} loading="lazy" decoding="async" fetchPriority="low" sizes="(min-width: 1024px) 25vw, (min-width: 640px) 50vw, 100vw" />
